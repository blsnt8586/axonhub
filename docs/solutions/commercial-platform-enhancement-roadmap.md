...
## Stage 28: Promo, Affiliate, And Self-Service Transaction Closure

Status: [x] Completed

Goal: close the customer-facing transaction loop across promo quoting,
checkout, wallet credit, subscription purchase, invite binding, rebate creation,
and rebate transfer, with backend accounting and browser behavior verified
together.

Defect resolved:

- [x] Fix `createMyEPayRechargeCheckout` so the GraphQL `promoCode` submitted by
  the existing frontend is forwarded to `PaymentService.CreateRechargeCheckout`.
- [x] Prevent the previous mismatch where the quote displayed a discounted
  payable amount but the generated ePay checkout still charged full price.

Backend closure:

- [x] Verify a `100 CNY` recharge with a `20 CNY` amount discount produces an
  `80 CNY` checkout while crediting the user's wallet with the original
  `100 CNY` recharge amount after verified payment notification.
- [x] Verify PromoUsage moves from reserved to applied and stores original,
  discount, and payable amounts.
- [x] Verify repeated payment notifications do not duplicate wallet credit,
  promo usage, or affiliate rebate rows.
- [x] Verify a `50 CNY` subscription with a 10% promo debits `45 CNY`, stores
  the original/discount/payable snapshot, and links an applied PromoUsage row.
- [x] Verify user-controlled subscription `userId` is ignored and the purchase
  remains bound to the authenticated user.
- [x] Verify affiliate rebates use the payable amount, not the original amount,
  and repeated transfers do not duplicate inviter wallet credit.

Frontend and browser closure:

- [x] Seed recharge and subscription promo codes plus an enabled affiliate
  setting through owner GraphQL operations.
- [x] Register an inviter, obtain the invite code, register an invitee, and bind
  the code through the real billing page.
- [x] Enter the subscription promo in the UI and verify it is sent in the
  purchase GraphQL variables.
- [x] Quote and submit the recharge promo in the UI, verify the quote payable is
  `1.50 CNY`, verify checkout variables contain the promo code, and complete
  the simulated ePay return flow.
- [x] Sign in as the inviter and transfer two rebates generated by the
  discounted subscription and recharge, totaling `0.60 CNY`.

Verification:

- [x] `go test ./internal/server/gql -run 'TestCommercialSelfService' -count=1`
- [x] `go test ./internal/server/gql ./internal/server/api ./internal/server/biz -count=1`
- [x] `pnpm exec tsc --noEmit`
- [x] `pnpm build`
- [x] `./scripts/e2e/e2e-test.sh commercial-smoke.spec.ts` (2 passed)
- [x] `./scripts/e2e/commercial-self-service-acceptance.sh`
...
| Stage 28 | this commit | 2026-07-10 | Fixed recharge promo forwarding and added backend/browser closure for promo discounts, subscription purchase, invite binding, affiliate rebates, and wallet transfer idempotency. |
