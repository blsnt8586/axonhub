...
type SubscriptionPlanQuery struct {
	...
	loadTotal                  []func(context.Context, []*SubscriptionPlan) error
...
func (_q *SubscriptionPlanQuery) prepareQuery(ctx context.Context) error {
	...
	if subscriptionplan.Policy == nil {
		return errors.New("ent: uninitialized subscriptionplan.Policy (forgotten import ent/runtime?)")
	}
	if err := subscriptionplan.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *SubscriptionPlanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SubscriptionPlan, error) {
	...
	for name, query := range _q.withNamedUserSubscriptions {
		if err := _q.loadUserSubscriptions(ctx, query, nodes,
			func(n *SubscriptionPlan) { n.appendNamedUserSubscriptions(name) },
			func(n *SubscriptionPlan, e *UserSubscription) { n.appendNamedUserSubscriptions(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *SubscriptionPlanQuery) loadUserSubscriptions(ctx context.Context, query *UserSubscriptionQuery, nodes []*SubscriptionPlan, init func(*SubscriptionPlan), assign func(*SubscriptionPlan, *UserSubscription)) error {
	...
	nodeids := make(map[int]*SubscriptionPlan)
	for i := range nodes {
...
func (_q *SubscriptionPlanQuery) WithNamedUserSubscriptions(name string, opts ...func(*UserSubscriptionQuery)) *SubscriptionPlanQuery {
	query := (&UserSubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
