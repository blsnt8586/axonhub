...
func (UsageDailyAggregate) Mixin() []ent.Mixin {
	return []ent.Mixin{
		TimeMixin{},
	}
}

func (UsageDailyAggregate) Indexes() []ent.Index {
	return []ent.Index{
		index.Fields("bucket_start").
			StorageKey("usage_daily_aggregates_by_bucket_start"),
		index.Fields("user_id", "bucket_start").
			StorageKey("usage_daily_aggregates_by_user_bucket"),
		index.Fields("project_id", "bucket_start").
			StorageKey("usage_daily_aggregates_by_project_bucket"),
		index.Fields("api_key_id", "bucket_start").
			StorageKey("usage_daily_aggregates_by_api_key_bucket"),
		index.Fields("channel_id", "bucket_start").
			StorageKey("usage_daily_aggregates_by_channel_bucket"),
		index.Fields("model_id", "bucket_start").
			StorageKey("usage_daily_aggregates_by_model_bucket"),
		index.Fields("bucket_start", "user_id", "api_key_id", "project_id", "channel_id", "model_id", "request_type", "status", "currency", "upstream_account_id").
			StorageKey("usage_daily_aggregates_unique_dimension").
			Unique(),
	}
}

func (UsageDailyAggregate) Fields() []ent.Field {
	return []ent.Field{
...
func (UsageDailyAggregate) Policy() ent.Policy {
