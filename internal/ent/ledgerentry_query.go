...
type LedgerEntryQuery struct {
	...
	loadTotal             []func(context.Context, []*LedgerEntry) error
...
func (_q *LedgerEntryQuery) QueryLedgerTransaction() *LedgerTransactionQuery {
...
func (_q *LedgerEntryQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *LedgerEntryQuery) FirstIDX(ctx context.Context) int {
...
func (_q *LedgerEntryQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *LedgerEntryQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *LedgerEntryQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *LedgerEntryQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *LedgerEntryQuery) WithLedgerTransaction(opts ...func(*LedgerTransactionQuery)) *LedgerEntryQuery {
	query := (&LedgerTransactionClient{config: _q.config}).Query()
...
func (_q *LedgerEntryQuery) prepareQuery(ctx context.Context) error {
	...
	if ledgerentry.Policy == nil {
		return errors.New("ent: uninitialized ledgerentry.Policy (forgotten import ent/runtime?)")
	}
	if err := ledgerentry.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *LedgerEntryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*LedgerEntry, error) {
	...
	if query := _q.withLedgerTransaction; query != nil {
		if err := _q.loadLedgerTransaction(ctx, query, nodes, nil,
			func(n *LedgerEntry, e *LedgerTransaction) { n.Edges.LedgerTransaction = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *LedgerEntryQuery) loadLedgerTransaction(ctx context.Context, query *LedgerTransactionQuery, nodes []*LedgerEntry, init func(*LedgerEntry), assign func(*LedgerEntry, *LedgerTransaction)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*LedgerEntry)
	for i := range nodes {
...
func (_q *LedgerEntryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(ledgerentry.Table, ledgerentry.Columns, sqlgraph.NewFieldSpec(ledgerentry.FieldID, field.TypeInt))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withLedgerTransaction != nil {
			_spec.Node.AddColumnOnce(ledgerentry.FieldLedgerTransactionID)
		}
	}
